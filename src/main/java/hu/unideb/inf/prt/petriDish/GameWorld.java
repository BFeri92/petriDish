...
public class GameWorld {
	...
	public GameWorld(WorldDescriptor descr) throws WeigthNumberNotMatchException
	{
		stepCount = 0;
		...
		{
			entities.add(
						new Agent(descr.getGenotypes().get(rng.nextInt(descr.getGenotypes().size() )),
								rng.nextInt(conf.getWorldSize()+1), rng.nextInt(conf.getWorldSize()+1),
								rng.nextInt(360))
						);
		}
		//adding food
	...
	public void step()
	{
		...
		while (it.hasNext())
		{
			...
			if (curr.getClass().equals(Agent.class))
			{
				...
				/*
				if (a.getxPos()<0)
					a.setxPos(Math.round(conf.getWorldSize()+a.getxPos()));
				if (a.getxPos()>conf.getWorldSize())
					a.setxPos(Math.round(a.getxPos()-conf.getWorldSize()));
				
				if (a.getyPos()<0)
					a.setyPos(Math.round(conf.getWorldSize()+a.getyPos()));
		...
		for (int i=0; i<entities.size(); i++ )
		{
			for (int j=0; j<entities.size(); j++)
			{
				...
				if (j!=i && e1.getClass().equals(Agent.class) && e2.getClass().equals(FoodEntity.class) && e1.colides(e2))
				{
					((Agent)e1).setHunger(((Agent)e1).getHunger()-((FoodEntity)e2).getSustenance());
					entities.remove(j);
					j--;
				}
			}
