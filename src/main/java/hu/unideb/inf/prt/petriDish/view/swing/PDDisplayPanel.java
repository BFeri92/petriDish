		...
		if (entityList!=null)
		{
			for (Entity e : entityList)
			{
				...
				if (e.getClass().equals(FoodEntity.class))
				{
					...
					graph.fillOval(eCenterX-e.radius, eCenterY-e.radius, 2*e.radius, 2*e.radius);
				}
				else if (e.getClass().equals(Agent.class))
				{
					...
					if (hunger>1)
						hunger=1;
					graph.setColor(new Color((int)Math.round(250*hunger), (int)Math.round(250*(1-hunger)), (int)0));
					graph.fillOval(eCenterX-e.radius, eCenterY-e.radius, 2*e.radius, 2*e.radius);
					graph.setColor(new Color(255,255,255));
					graph.drawLine(eCenterX, eCenterY, (int)Math.round(eCenterX + e.radius*Math.cos(rad)), (int)Math.round(eCenterY + e.radius*Math.sin(rad)));
				}
			}
		}
	}
