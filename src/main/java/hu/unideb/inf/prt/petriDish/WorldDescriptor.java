...
public class WorldDescriptor {
	...
	static private Logger logger = LoggerFactory.getLogger(WorldDescriptor.class);
	...
	public WorldDescriptor(GameConfiguration conf)
	{
		...
		{
			genotypes.add(new Genotype(conf.getNodesPerHiddenLayer(), conf.getHiddenLayers()));
		}
	}
	
	public WorldDescriptor(List<Genotype> genBase, GameConfiguration conf) throws GenomSizeNotMatchException
	{
		genotypes = new Vector<Genotype>(conf.getAgentCount());
		...
		for (int i=0; i<conf.getAgentCount(); i++)
		{
			...
			if (conf.getMutate())
				genotypes.get(genotypes.size()-1).mutate(conf.getMutationProb(), conf.getMutationAmount());
		}
	}
